...
class SyncPrivacySettingsImpl : SyncPrivacySettings{
    ...
    override fun scheduleEditPrivacySettings(privacySettingsRequestData: PrivacySettingsRequestData) {
        ...
        data.putString("EDIT_PRIVACY_SETTINGS_REQUEST_DATA", serializeEditPrivacySettingsRequestData(privacySettingsRequestData))
        ...
        workManager.enqueueUniqueWork(SYNC_EDIT_PRIVACY_SETTINGS_ID, ExistingWorkPolicy.REPLACE, work)
    }

    override fun scheduleEditDuelsExceptions(editDuelsExceptionsRequestData: EditDuelsExceptionsRequestData) {
        val data = Data.Builder()
        data.putString("EDIT_DUELS_EXCEPTIONS_REQUEST_DATA", serializeEditDuelsExceptionsRequestData(editDuelsExceptionsRequestData))
        ...
        val work = OneTimeWorkRequest.Builder(SyncEditDuelsExceptionsWorker::class.java)
            ...
            .build()
        val workManager = WorkManager.getInstance(NetworkModule.context)
        workManager.enqueueUniqueWork(SYNC_EDIT_DUELS_EXCEPTIONS_ID, ExistingWorkPolicy.REPLACE, work)
    }

    class SyncEditPrivacySettingsWorker(val context: Context, val params: WorkerParameters) : CoroutineWorker(context, params){
        override suspend fun doWork(): Result {
            val json = inputData.getString("EDIT_PRIVACY_SETTINGS_REQUEST_DATA")

            val editPrivacySettingsRequestData = json?.let { deserializeEditPrivacySettingsRequestData(it) }
    ...
    class SyncEditDuelsExceptionsWorker(val context: Context, val params: WorkerParameters) : CoroutineWorker(context, params){
        override suspend fun doWork(): Result {
            val json = inputData.getString("EDIT_DUELS_EXCEPTIONS_REQUEST_DATA")

            val editDuelsExceptionsRequestData = json?.let { deserializeEditDuelsExceptionsRequestData(it) }
        ...
        private fun deserializeEditDuelsExceptionsRequestData(json: String): EditDuelsExceptionsRequestData {
            return Gson().fromJson(json, TypeToken.get(EditDuelsExceptionsRequestData::class.java).type)
        }
    }
