...
class AcceptFriendshipDialogAlertViewModel : ViewModel(){
    ...
    fun acceptFriendshipRequest(userIdToken: UserIdTokenData, userData: UserData){
        addToFriendsJob = GlobalScope.launchExt(addToFriendsJob){
            ...
            if(result is FriendActionStatus.Success)
                addFriendStatus.postValue(UsersInteractor.FriendActionState.FriendshipRequestIsAccepted(userData.apply {
    ...
    fun rejectFriendshipRequest(userIdToken: UserIdTokenData, userData: UserData){
        addToFriendsJob = GlobalScope.launchExt(addToFriendsJob){
            val result = friendsInteractor.rejectFriendship(FriendActionRequestData(userIdToken, userData.userId))
            if(result is FriendActionStatus.Success)
                addFriendStatus.postValue(UsersInteractor.FriendActionState.FriendshipRequestIsRejected(userData.apply {
