package com.example.studita.presentation.views.crop_view;
...
public class CropView extends FrameLayout implements CropAreaView.AreaViewListener, CropGestureDetector.CropGestureListener {
    ...
    public void reset(boolean recreate) {
        ...
        state.reset(areaView, 0, freeform, recreate);
    ...
    private void fillAreaView(RectF targetRect, boolean allowZoomOut) {
        ...
        final float y = (targetRect.centerY() - (imageView.getHeight() - bottomPadding) / 2) / areaView.getCropHeight() * state.getOrientedHeight();
    ...
    public void rotate90Degrees() {
        ...
        state.reset(areaView, orientation, fform, false);
        updateMatrix();
    ...
    public void onScale(float scale, float x, float y) {
        ...
        float pivotY = (y - (imageView.getHeight() - bottomPadding) / 2) / areaView.getCropHeight() * state.getOrientedHeight();
