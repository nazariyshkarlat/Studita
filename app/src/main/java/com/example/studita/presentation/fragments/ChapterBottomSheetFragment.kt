        ...
        chapterViewModel?.let {
            ...
            it.progressState.observe(viewLifecycleOwner, Observer { progress ->
                if (progress) {
                    ...
                    chapterLayoutRecyclerView.adapter =
                        chapterUiModel?.let { model -> ChapterPartsAdapter(model) }
            ...
            if (snackbarShowReason != SnackbarShowReason.NONE) {
                activity?.let {
                    ...
                    val text = when (snackbarShowReason) {
                        ...
                        SnackbarShowReason.CHAPTER_COMPLETED_AND_BAD_RESULT -> "${resources.getString(
                            R.string.exercise_snackbar_chapter_completed_reason
                        )} ${resources.getString(R.string.exercise_snackbar_bad_result_reason)}"
                        else -> throw IOException("unknown show snackbar reason")
                    }

                    snackbar.show(
                        text,
                        ContextCompat.getColor(snackbar.context, R.color.green),
