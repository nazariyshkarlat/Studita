...
class UserStatYesterdayFragment : UserStatPageFragment(){
            ...
            if(savedInstanceState == null){
                PrefsUtils.getUserId()?.let { userId ->
                    PrefsUtils.getUserToken()?.let { userToken ->
                        it.getUserStatistics(userId, userToken, UserStatisticsTime.YESTERDAY)
                    }
                }
            }
            it.userStatisticsYesterdayState.observe(viewLifecycleOwner, Observer { state ->
