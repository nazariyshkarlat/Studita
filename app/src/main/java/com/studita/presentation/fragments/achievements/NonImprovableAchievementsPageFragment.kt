...
class NonImprovableAchievementsPageFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        return RecyclerView(container!!.context).apply {
            ...
            clipToPadding = false
            id = R.id.nonImprovableAchievementsRecyclerView
            updatePadding(bottom = (if(activity is MainActivity) resources.getDimension(R.dimen.bottomNavigationHeight).toInt() else 0)+8F.dp)
            isSaveEnabled = false
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        lifecycleScope.launchWhenStarted {
            (parentFragment as AchievementsFragment).achievementsViewModel.nonImprovableAchievementsState.collect {
                (view as RecyclerView).adapter = AchievementsAdapter(it, false)
            }
        }
        ...
        lifecycleScope.launchWhenStarted {
            (parentFragment as AchievementsFragment).achievementsViewModel.recyclerViewScrollToItemEvent.collect {
                if(it.first == AchievementsViewModel.AchievementsCategory.NON_IMPROVABLE) {
