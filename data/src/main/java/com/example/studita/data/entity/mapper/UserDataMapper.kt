...
class UserDataDataMapper : Mapper<UserDataEntity, UserDataData>{
    ...
    override fun map(source: UserDataEntity) = UserDataData(source.userName, source.userFullName, source.userPublicId, source.avatarLink, source.currentLevel, source.currentLevelXP, source.streakDays, source.isSubscribed, ArrayList(source.completedParts), DateTimeFormat().parse(source.streakDatetime), source.todayCompletedExercises)

}

class UserDataEntityMapper : Mapper<UserDataData, UserDataEntity>{

    override fun map(source: UserDataData) = UserDataEntity(source.userName, source.userFullName, source.userPublicId, source.avatarLink, source.currentLevel, source.currentLevelXP, source.streakDays, source.isSubscribed, ArrayList(source.completedParts), DateTimeFormat().format(source.streakDatetime), source.todayCompletedExercises)
